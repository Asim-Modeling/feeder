...
CONTEXT_SCHEDULER_CLASS::CONTEXT_SCHEDULER_CLASS (ASIM_MODULE parent, const
    ...
    : ASIM_MODULE_CLASS(parent,name),
      ...
      numOldHWC2(0),
      rrHWC(1),
      deschedulingActive(false),
      activeContextSwitch(false),
      supressCS(0), CSdeferred(false)

{
    ...
    statIntervalScheds = 0;
    RegisterState(&statIntervalScheds, "IntervalScheds",
                  "Number of times interval timer scheduling took place");
    statSwapScheds = 0;
    RegisterState(&statSwapScheds, "SwapScheds",
                  "Number of times swap scheduling took place (due to unavailable exec classes)");
   
    cyclesCsSupressed = 0;
    RegisterState(&cyclesCsSupressed, "cyclesCsSupressed",
                  "Number of cycles context switch swap supressed due to too many migration attempts");


    // Initialize sub classes
    coreSwapTrigger = new CONTEXT_TRIG_CLASS();
}
...
CONTEXT_SCHEDULER_CLASS::AddSoftwareContext (SW_CONTEXT swc)
{
    ...
    for (UINT32 i=0; i<MAX_NUM_SWCS; i++)
    {
	    ...
	    swc->runningHWCId = (UINT64) swc->GetHWCNum();
	    
	    IFEEDER_STREAM_HANDLE streamHandle = swc->GetFeederStreamHandle();
	    // the thread is associated with top-most feeder (archlib), get the handle to the child (LIT).
	    if (swc->GetIFeeder()->GetChildFeeder())
	    {
		char statArg[1024];		
		const char *traceName = swc->GetIFeeder()->GetChildFeeder()->GetTraceName(streamHandle);
		sprintf(statArg, "SWC %s committed", traceName);
		RegisterState(&(swc->nMicroCommitted), statArg, swc->IDToString().c_str());

		sprintf(statArg, "SWC %s running on HWC", traceName);
		RegisterState(&(swc->runningHWCId), statArg, swc->IDToString().c_str());
	    }
	    else
	    {
		RegisterState(&(swc->nMicroCommitted), "SWC NOTRACENAME committed", swc->IDToString().c_str());
		RegisterState(&(swc->runningHWCId), "SWC NOTRACENAME running on HWC", swc->IDToString().c_str());
	    }
            return;
        }
    }
...
CONTEXT_SCHEDULER_CLASS::ContextSwitchCompleted(SW_CONTEXT swc)
{
    ...
    if (swc->GetMappedToHWC() == false)
    {
        ...
        ASIMERROR( "CONTEXT_SCHEDULER_CLASS::ContextSwitchCompleted "
                   << "called when software context is not mapped to a hardware context." << endl);
    }
    ...
    swc->SetNoHWC();
    swc->SetNotFirstInstructionFetched();
...
//        delete swc;
    ...
    {
	// Have to make this hardware context free in the 
	// case of core swapping 
	hwc->SetNoSWC();
    }
}
...
CONTEXT_SCHEDULER_CLASS::IntervalTimer(HW_CONTEXT hwc)
{
    ...
    {
	cout << "Warning: Context switch ignored for hwc: " << hwc << " and swc: " << oldSWC << endl;
    ...
    if (CORE_SWAP_DEBUG) 
    {
	cout << "Going to initiate context switch for hwc: " << hwc << " and swc: " << oldSWC << endl;
    }
...
CONTEXT_SCHEDULER_CLASS::Clock(UINT64 cycle)
{
    ...
    /*
    // multiple clock callbacks with cycle number 1.
    if (cycle == 1)
    {
	for (UINT32 i = 0; i < numHWC; i++)
	{
	    HW_CONTEXT hwc = HWContextList[i];
	    if (hwc != ASIM_HWC_NONE)
	    {
		SW_CONTEXT swc = hwc->GetSWC();		
		if (swc != ASIM_SWC_NONE)
		{
		    RegisterState(&(swc->nMicroCommitted), "SWC micro committed", "SWC micro committed instructions");
		}
	    }
	}
    }
...
//        CheckForNewStream();
    ...
    {
	if (ENABLE_CS_SUPRESS) 
	{
	    while ((!csEventQueue.empty()) && (-csEventQueue.top() <= (INT64)cycle))
	    {
		csEventQueue.pop();
	    }
	    
	    if (csEventQueue.size() >= CS_EVENTLIMIT)
	    {
		supressCS = cycle + CS_SUPRESS_CYCLES;
		cyclesCsSupressed++;
	    }
	}

	// okhan: This is the "trigger" and "response" mechanism. 
        // shantanu: Trigger class now also stores the hwc number that wants 
        //           its swc to be descheduled. Trigger type could be of two
        //           types: 1) interval trigger, 2) fub unavailability trigger
	// TRIGGER
	reschedule = coreSwapTrigger->computeTrigger(cycle, HWContextList, numHWC);
	// RESPONSE
	HandleTrigResponse(currentCycle, reschedule);
    }
}

// WARNING: 
//        This assumes that the newly selected hardware context is always free of any
//        software context.  Best used to round-robin a SW among multiple HWCs.

/*
  kkrangan - CS code is distributed in feeder, x86 and possibly in the
  timing model as well.  When reschedule is set, core swapping is initiated.
  1) activeContextSwitch and deschedulingActive are set to true; 2) SWC is
  put to context_switch_requested state; 3) while in that state, when the
  timing model calls fetch, SWC feeds a bogus 'contextswitch' uop down the
  pipe, and transitions to context_switch_sent state; 3) when the bogus uop
  commits, SWC transitions to context_switch_none state.  The context
  scheduler uses that to de-assert deschedulingActive indicating that
  pipeline has drained in the timing model. 4) after context_transfer
  cycles, used to mimic architectural state-transfer overhead, the SWC is
  re-started on a new HWC.
*/

void CONTEXT_SCHEDULER_CLASS::DoIntervalSched(UINT64 currentCycle, bool reschedule)
{
    HW_CONTEXT hwc;
    HW_CONTEXT oldHWC1;
    ...
    // Run this block if a trigger is called, and no activeContextSwitch is
    // underway.
    if (reschedule && !activeContextSwitch) 
    {
	// Find active HWC 
	for (UINT32 i=0; i < numHWC; i++) 
	{
	    if (IsHWCActive(i)) 
	    {
		oldHWC1 = HWContextList[i];
		numOldHWC1 = i;
		oldSWC1 = oldHWC1->GetSWC();
                oldSWC1->IncSwapCount();

		IntervalTimer(oldHWC1);		

		activeContextSwitch = true;		
		deschedulingActive = true;
		contextSwitchStartCycle = currentCycle;
		
		statIntervalScheds++;		
		
		cout << "CS start @ cycle: " << contextSwitchStartCycle << ", HWC: " << numOldHWC1 
		     << ", SWC: " << oldSWC1->IDToString() << endl;
                break;
	    }
	}
    }
    
    // wait for descheduling to finish and then add up transfer latency
    if (activeContextSwitch && deschedulingActive && (!oldSWC1->GetDeschedulePending())) 
    {
        deschedulingActive = false;
        contextSwitchEndCycle = currentCycle + CONTEXT_TRANSFER_CYCLES;
    }
    
    // When swc deschedule is complete and transfer is done, pick next HWC to run
    if (activeContextSwitch && !deschedulingActive && (currentCycle >= contextSwitchEndCycle))
    {
	// Determine new HWC to run the oldSWC: 
        numNewHWC = (numOldHWC1 + 1) % numHWC;
	
	// Now that we have the new HWC, run it!
	hwc = HWContextList[numNewHWC];
	
	if (oldSWC1 != ASIM_SWC_NONE) 
	{
	    cout << "CS end @ cycle: " << currentCycle << ", HWC: " << numOldHWC1 << ", moved to HWC: " << numNewHWC
		 << ", SWC: " << oldSWC1->IDToString() << endl;
	    RunSoftwareContext(oldSWC1, hwc);
	}
	else {
	    hwc->SetNoSWC();
	    statContextSwitchNoSWC++;
	}
	// Clear activeContextSwitch when new HWC kicked off
	activeContextSwitch = false;
        hwc->ResetCsHold(); // mdpowel1 for sanity check
	
        // print context map         
        if (CORE_SWAP_DEBUG) 
	{
	    cout << "Context mapping after rescheduling which took " << (currentCycle - contextSwitchStartCycle) << " cycles" << endl;
	    for (UINT32 i=0; i < numHWC; i++) 
	    {
		cout << "Index " << i << " HWC " << HWContextList[i] << " SWC " << HWContextList[i]->GetSWC() << endl;
	    }
	}
    }
    return;
...
void CONTEXT_SCHEDULER_CLASS::DoIntervalSwap(UINT64 currentCycle, bool reschedule)
{
    HW_CONTEXT temp_hwc;
    
    // Run this block if a trigger is called, and no activeContextSwitch is underway.
    if (reschedule && !activeContextSwitch) 
    {
	vector<UINT32> pickedHWC; pickedHWC.clear();
	
	swapPair.clear(); swapPairSWC.clear(); swapEndCycle.clear();
	
	for (UINT32 i = 0; i < numHWC; i++)
	{
	    if (IsHWCActive(i) && (find(pickedHWC.begin(), pickedHWC.end(), i) == pickedHWC.end()))
	    {
		// pick first hwc in swap
		numOldHWC1 = i;
		temp_hwc = HWContextList[numOldHWC1];
		oldSWC1 = temp_hwc->GetSWC(); 
		ASSERTX(oldSWC1 != ASIM_SWC_NONE);
		
		// initiate context switch in SWC1
		IntervalTimer(temp_hwc);
		
		// pick second hwc in swap
		numOldHWC2 = (numOldHWC1 + 1) % numHWC;
		temp_hwc = HWContextList[numOldHWC2];
		oldSWC2 = temp_hwc->GetSWC(); 
		ASSERTX(oldSWC2 != ASIM_SWC_NONE);
		ASSERTX(find(pickedHWC.begin(), pickedHWC.end(), numOldHWC2) == pickedHWC.end());

		// initiate context switch in SWC2
		IntervalTimer(temp_hwc);

		pickedHWC.push_back(numOldHWC1);
		pickedHWC.push_back(numOldHWC2);		

		// cs bookkeeping
		std::pair<SW_CONTEXT, SW_CONTEXT> psw(oldSWC1, oldSWC2);		
		std::pair<UINT32, UINT32> p(numOldHWC1, numOldHWC2);
		swapPair.push_back(p);
		swapPairSWC[p] = psw;
		...
		statIntervalSwaps++;
		cout << "CS start @ cycle: " << contextSwitchStartCycle << " between HWC's " << numOldHWC1 << " <-> " << numOldHWC2 << endl;
	    }
	}
    }
    
    // is descheduling for some swaps?
    if (activeContextSwitch && swapPair.size())
    {
	vector<csSwapPair>::iterator vit = swapPair.begin(); 
	while (vit != swapPair.end())
	{
	    map<csSwapPair, csSwapSWC>::iterator mit = swapPairSWC.find(*vit);
	    oldSWC1 = mit->second.first;
	    oldSWC2 = mit->second.second;
	    
	    if (!(oldSWC1->GetDeschedulePending() || oldSWC2->GetDeschedulePending()))
	    {
		swapEndCycle[*vit] = currentCycle + CONTEXT_TRANSFER_CYCLES;
    ...
    // is transfer pending for some swaps?    
    if (activeContextSwitch && swapEndCycle.size())
    {
	map<csSwapPair, UINT64>::iterator eit = swapEndCycle.begin();
	while (eit != swapEndCycle.end())
	{
	    if (currentCycle >= eit->second)
	    {
		numOldHWC1 = eit->first.first;
		numOldHWC2 = eit->first.second;

		HW_CONTEXT temp_hwc1 = HWContextList[numOldHWC1];
		HW_CONTEXT temp_hwc2 = HWContextList[numOldHWC2];

		map<csSwapPair, csSwapSWC>::iterator mit = swapPairSWC.find(eit->first);
		ASSERTX(mit != swapPairSWC.end());
		oldSWC1 = mit->second.first;
		oldSWC2 = mit->second.second;

		RunSoftwareContext(oldSWC1, temp_hwc2);
		RunSoftwareContext(oldSWC2, temp_hwc1);

		swapEndCycle.erase(eit);
		eit = swapEndCycle.begin();

		cout << "CS end @ cycle: " << currentCycle << " between HWC's " << numOldHWC1 << " <-> " << numOldHWC2 
		     << ", took " << (currentCycle - contextSwitchStartCycle) << " cycles" << endl;
	    }
	    else 
	    {
		eit++;
	...
	if (swapEndCycle.size() == 0 && swapPair.size() == 0)
	{
	    activeContextSwitch = false;
	    swapPairSWC.clear();
...
void CONTEXT_SCHEDULER_CLASS::DoPVDefectTrig(UINT64 currentCycle, bool reschedule)
{
    HW_CONTEXT temp_hwc;
	...
	{
	    if ((IsHWCActive(i)) && 
		(find(pickedHWC.begin(), pickedHWC.end(), i) == pickedHWC.end()) && 
		(HWContextList[i]->GetTriggerContextSwitch()))
	    {
		// pick first hwc in swap
		numOldHWC1 = i;
		temp_hwc = HWContextList[numOldHWC1];
...
// Swaps the swcs between two hwcs. This is done on behest of the hwc
// requesting a deschedule due to unavailibility of a certain execution
// class(es).
void CONTEXT_SCHEDULER_CLASS::DoSwapSched(UINT64 currentCycle, bool reschedule)
{
    HW_CONTEXT temp_hwc;
	...
	// Set activeContextSwitch bit when reschedule initiated
	activeContextSwitch = true;
	
	// Old HWC
	numOldHWC1 = coreSwapTrigger->GetTriggerHWC();
	temp_hwc = HWContextList[numOldHWC1];
	oldSWC1 = temp_hwc->GetSWC(); 
	
        if (CORE_SWAP_DEBUG)
	{
	    cout << "Context switch start cycle: " << contextSwitchStartCycle << endl;
	    cout << "Rescheduling numHWC " << numOldHWC1 << " HWC " << temp_hwc << " SWC " << oldSWC1 << endl;
	}
        // IntervalTimer is the function which deschedules the software context from the
        // supplied hardware context. Basically, it initiates the actual
        // context switching.
        IntervalTimer(temp_hwc);

         if (currentCycle < supressCS) 
	 {
	     CSdeferred = true;
	     
	     // clear cs event queue
	     while (!csEventQueue.empty()) csEventQueue.pop();

	     if (CORE_SWAP_DEBUG)	     
	     {
		 cout << "Deferring CS swap at " << currentCycle << " until " << supressCS;
	     }
	 } 
	 else 
	 {
	     // New HWC
	     numOldHWC2 = PickHWCForSwap(numOldHWC1);
	     ...
	     if (CORE_SWAP_DEBUG)
	     {
		 cout << "Rescheduling numHWC " << numOldHWC2 << " HWC " << temp_hwc << " SWC " << oldSWC2 << endl;
	     }
	     if (oldSWC2 != ASIM_SWC_NONE) 
	     {
		 IntervalTimer(temp_hwc);
	     }
	 }
    ...
    // deferred CS
    if ((CSdeferred) && (currentCycle >= supressCS))
    {
	CSdeferred = false;
	...
	if (CORE_SWAP_DEBUG)
	{
	    cout << "Deferral complete.  Rescheduling numHWC " << numOldHWC2 << " HWC " << temp_hwc << " SWC " << oldSWC2 << endl;
	}
	if (oldSWC2 != ASIM_SWC_NONE) 
	{
	    IntervalTimer(temp_hwc);
    ...
    // wait for descheduling to finish and then add up transfer latency
    if (!CSdeferred && activeContextSwitch && deschedulingActive && !(oldSWC1->GetDeschedulePending() || 
								      (oldSWC2 == ASIM_SWC_NONE ? 0 : oldSWC2->GetDeschedulePending())))
    {
        deschedulingActive = false;
        contextSwitchEndCycle = currentCycle + CONTEXT_TRANSFER_CYCLES;
    }
    ...
    if (activeContextSwitch && !deschedulingActive && (currentCycle >= contextSwitchEndCycle)) 
    {
        // Hook up oldSWC1 to numOldHWC2
	temp_hwc = HWContextList[numOldHWC2];
	ASSERTX(oldSWC1 != ASIM_SWC_NONE); // this is the SWC that triggered core swap
	if (CORE_SWAP_DEBUG)
	{
	    cout << "CYCLE: " << currentCycle << "  Scheduling " << oldSWC1->IDToString() 
		 << " on HWC " << temp_hwc << " and numHWC " << numOldHWC2 << endl;
	}
	RunSoftwareContext(oldSWC1, temp_hwc);
        temp_hwc->ResetCsHold(); // mdpowel1 for sanity check
	
        // Hook up oldSWC2 (if any) to numOldHWC1
        temp_hwc = HWContextList[numOldHWC1];
        if (oldSWC2 != ASIM_SWC_NONE) 
        {
	    if (CORE_SWAP_DEBUG)
	    {
		cout << "CYCLE: " << currentCycle << "  Scheduling " << oldSWC2->IDToString() 
		     << " on HWC " << temp_hwc << " and numHWC " << numOldHWC1 << endl;
	    }
	    RunSoftwareContext(oldSWC2, temp_hwc);
        }
	else 
        {
	    temp_hwc->SetNoSWC();
...
// Picks up a new hwc for scheduling swc. This is called by the DoSwapSched()
// upon receiving ECLASS_UNAVAILABLE_TRIG
UINT32 CONTEXT_SCHEDULER_CLASS::PickHWCForSwap(UINT32 oldhwc) 
{
    UINT32 newhwc;
    bool foundfirst = false;
    
    switch (CS_SELECTION_ALGORITHM) 
    {
	case 0:
	    // Scheme 0: choose the next core
	    newhwc = (oldhwc + NUM_HWCS_PER_CPU) % numHWC;
	    break;
	    
	case 1:
	    // Scheme 1: choose the next round robin core, and make sure that it is not
	    // the same as the oldhwc.
	    if (rrHWC != oldhwc) {
		newhwc = rrHWC;
		rrHWC = (rrHWC + NUM_HWCS_PER_CPU) % numHWC;
	    }
	    else {
		newhwc = (rrHWC + NUM_HWCS_PER_CPU) % numHWC;
		rrHWC = (rrHWC + 2*NUM_HWCS_PER_CPU) % numHWC;
	    }
	    break;
	    
	case 2:
	    // Scheme 2: choose the core with the SWC that has been booted due to defects
	    //           the fewest number of times. mdpowel1
	    UINT32 i, minswaps, minswaphwc;
	    SW_CONTEXT swc; 
	    for (i = 0; i < numHWC; i++) 
	    {
		swc = HWContextList[i]->GetSWC();
		if (swc == ASIM_SWC_NONE) 
		{
		    continue;
		}
		...
		if (!foundfirst) 
		{
		    minswaps = swc->GetSwapCount();
		    minswaphwc = i;
		    foundfirst = true;
		} 
		else if (swc->GetSwapCount() < minswaps)
		{
		    minswaps = swc->GetSwapCount();
		    minswaphwc = i;
		}
	    ...
	    ASSERT(foundfirst,"found no sw contexts on any hw context"); // check that there is a swc on some hwc 
	    newhwc = minswaphwc;
	    break;
	    
	default:
	    ASSERT(0,"invalid CS_SELECTION_ALGORITHM");
    }
    return newhwc;
}

void CONTEXT_SCHEDULER_CLASS::HandleTrigResponse(UINT64 currentCycle, bool reschedule) 
{
    // A trigger can not be generated (reschedule) while a context switch is
    // active (activeContextSwitch); i.e. both cannot be true simultaneously
    // except in the same cycle when the trigger is generated.
    // Shantanu
    
    // okhan_cs: Move this assertion for ECLASS_UNAVAILABLE_TRIG only
    // ASSERTX(!(reschedule && activeContextSwitch) || contextSwitchStartCycle == currentCycle);

    switch (coreSwapTrigger->GetTriggerType()) 
    {
        case INTERVAL_TRIG:
            DoIntervalSched(currentCycle, reschedule);
            break;
	    
	case SWAP_INTERVAL_TRIG:
	    DoIntervalSwap(currentCycle, reschedule);
	    break;

	case PV_DEFECT_TRIG:
	    DoPVDefectTrig(currentCycle, reschedule);
	    break;
	    
        case ECLASS_UNAVAILABLE_TRIG:
            ASSERTX(!(reschedule && activeContextSwitch) || contextSwitchStartCycle == currentCycle);
	    
	    if (reschedule)
	    {
		csEventQueue.push(-(currentCycle + EVENT_WINDOWSIZE - 1)); 
		if (CORE_SWAP_DEBUG)
		{
		    cout << "at Cycle " << currentCycle << " pushing onto CS event queue, now size " << csEventQueue.size() << "\n";
		}
	    }
	    DoSwapSched(currentCycle, reschedule);
            break;
	    
        default:
            ASIMERROR("CONTEXT_SCHEDULER_CLASS::HandleTrigResponse is trying to use an unknown core swap trigger type " << endl);
    }
    return;
}
...
CONTEXT_SCHEDULER_CLASS::IsHWCActive(UINT32 hwc_num) const
{
    ...
    {
	// okhan: I'm adding the checks for HWC and SWC to make 
        // sure the scheduler only considers the hwc_num
	// that has a HWC active and the SWC is mapped to it!
	
	SW_CONTEXT swc = HWContextList[hwc_num]->GetSWC();
	HW_CONTEXT hwc = HWContextList[hwc_num];
	//  cout << "HWC # " << hwc_num << " : HWC " << hwc 
        // << " is mapped to SWC " << swc << endl;
	
	if (!swc) {
	    //	    cout << "HWC # " << hwc_num << " : SWC " << swc 
            //      << " is mapped to HWC NONE" << endl;
	    return false;
	}
	// cout << "HWC # " << hwc_num << " : SWC " << swc 
        // << " is mapped to HWC " << swc->GetHWC() << endl;
