...
class CONTEXT_SCHEDULER_CLASS : public ASIM_MODULE_CLASS
{
  public:
    ...
    // for core swapping
    void DoIntervalSched(UINT64 currentCycle, bool reschedule);
    void DoIntervalSwap(UINT64 currentCycle, bool reschedule);
    void DoPVDefectTrig(UINT64 currentCycle, bool reschedule);
    void DoSwapSched(UINT64 currentCycle, bool reschedule);
    UINT32 PickHWCForSwap(UINT32 oldhwc);
    void HandleTrigResponse(UINT64 currentCycle, bool reschedule);
...
//    bool UnblockThread (ASIM_THREAD thread);
  ...
  private:
    ...
    // Context Trigger for core swapping
    CONTEXT_TRIG coreSwapTrigger;
    ...
    // core swapping variables
    SW_CONTEXT  oldSWC1;
    SW_CONTEXT  oldSWC2;
    UINT32 	numOldHWC1;
    UINT32 	numOldHWC2;
    UINT32      rrHWC;                 // used for round robin hw context selection
    bool        deschedulingActive;    // software context descheduling is active
    bool 	activeContextSwitch;   // Context switch is in progress
    UINT64      contextSwitchStartCycle; // cycle when the context switch was started
    UINT64      contextSwitchEndCycle;   // cycle when the context switch will end. This is known
                                         // after the descheduling is over.

    // kkrangan: allow multiple CS swaps per cycle
    typedef std::pair<UINT32, UINT32> csSwapPair;
    typedef std::pair<SW_CONTEXT, SW_CONTEXT> csSwapSWC;

    vector<csSwapPair> swapPair;
    map<csSwapPair, csSwapSWC> swapPairSWC;
    map<csSwapPair, UINT64> swapEndCycle;

    priority_queue<INT64> csEventQueue;
    UINT64 supressCS;
    ...
    UINT64 statSWCRunnableNoHWC;       ///< SWC became runnable but no hwc
                                       ///< was available   
    UINT64 statIntervalScheds;         // number of times interval scheduling took place
    UINT64 statIntervalSwaps;          // number of times interval swaps took place
    UINT64 statSwapScheds;             // number of times swap scheduling took place 
    UINT64 cyclesCsSupressed;
    UINT64 CSdeferred;
