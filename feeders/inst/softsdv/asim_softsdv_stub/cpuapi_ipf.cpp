...
CPU_INFO_CLASS cpuInfo[MAX_CPU_DEVICES];

//
// Offsets to useful values off AR6, the process context pointer inside the
// simulated OS.  These vary by OS and are passed in by the script that
// starts a simulation.
//
static struct
{
    int pidOffset;
    int nameOffset;
    UINT64 timerInterruptVector;
    UINT64 idleLoopInstrTag;    // One instruction representing the idle
                                // loop in the kernel.  Useful for studying
                                // power impact of a PAUSE instruction.
}
contextInfo;
...
)
{
    ...
    switch (*regType)
    {
      ...
      default:
        ERROR("Unexpected register type");
        break;
    }
...
CPU_HISTORY cpuHistory[MAX_CPU_DEVICES];
    ...
    ASIM_SOFTSDV_INST_INFO_CLASS * inst)
{
    ...
    if (contextInfo.pidOffset != -1)
    {
        ASSERTX(cpuInfo[cpuNum].ReadMemory(task_struct_base + contextInfo.pidOffset,
                                           sizeof(pid), &pid));
    }
    ...
    comm[0] = 0;
    comm[17] = 0;
    if (contextInfo.nameOffset != -1)
    {
        ASSERTX(cpuInfo[cpuNum].ReadMemory(task_struct_base + contextInfo.nameOffset,
                                           16, comm));
    }
            ...
            cpuapi_u64_t *actual)
{
    ...
    if (asim_io.AsimPid())
    {
        ...
        {
            ASIM_SOFTSDV_FETCH_REQUEST fetchRequest;

            fetchRequest = asim_io.FetchRequestQueue().OpenNext();
            ASSERT(fetchRequest != NULL, "Remote Asim process exited");
            fetchCpuNum = fetchRequest->CpuNum();
            fetchInstrsRemaining = fetchRequest->NInstrs();

            asim_io.FetchRequestQueue().Close(fetchRequest);
        }
    ...
    while ((fetchInstrsRemaining != 0) && ! earlyTermination)
    {
        ...
        for (unsigned int i = 0; i < nInstrsToFetch; i++)
        {
            ...
            if (instInfo[i].GetVirtAddr() == contextInfo.timerInterruptVector)
            {
                inst->SetTimerInterrupt(true);
            }

            if (instInfo[i].GetVirtAddr() == contextInfo.idleLoopInstrTag)
            {
            ...
            if (instInfo[i].GetVirtAddr() >= 0xe000000000000000LL)
            {
                ...
                if (decode->dst1.type == EM_DECODER_REGISTER)
                {
                    ...
                    //
                    if ((contextInfo.timerInterruptVector == 0) &&
...
void
AsimInitCpu_ArchSpecific(UINT32 cpuNum)
{
    ...
    contextInfo.pidOffset = -1;
    contextInfo.nameOffset = -1;
    contextInfo.timerInterruptVector = 0;
    contextInfo.idleLoopInstrTag = 0;
...
//
// Learn offsets from process info pointer to useful information
//
void
ProcessContextLocation(
    int pidOffset,
    int nameOffset)
{
    contextInfo.pidOffset = pidOffset;
    contextInfo.nameOffset = nameOffset;
    printf("Process context:  Pid offset %d, Name offset %d\n", pidOffset, nameOffset);
}
...
TimerInterruptVector(
    UINT64 vector)
{
    contextInfo.timerInterruptVector = vector;
    printf("Timer interrupt vector:  0x%016llx\n", vector);
}
...
IdleLoopInstrTag(
    UINT64 idleAddr)
{
    contextInfo.idleLoopInstrTag = idleAddr;
    printf("Idle loop (PAUSE instruction):  0x%016llx\n", idleAddr);
}
