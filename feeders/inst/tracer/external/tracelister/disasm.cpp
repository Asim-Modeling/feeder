...
CHAR *decode_memop(UINT32 encoded, UINT64 pc, INT32 *src1, INT32 *src2, INT32 *offset)
{
  UINT32 given;
  alpha_insn *insn;
  INT32 found = 0;
...
CHAR *disassemble(UINT32 encoded, UINT64 pc)
{
  UINT32 given;
  alpha_insn *insn;
  INT32 found = 0;
  ...
  */
  ...
  for(insn = alpha_insn_set; insn->name && !found; insn++) {
	 ...
	 (given & MEMORY_BRANCH_FORMAT_MASK))
	{
	  if(given & (1<<15))
	  ...
	  else
	    {
	      ...
	      sprintf(buffer, "%-9s%s, (%s), 0x%lx%lx",
		      insn->name,
		      alpha_int_regs[RA(given)],
		      alpha_int_regs[RB(given)],
		      (branch_target>>32) & 0xffffffffL,
	 ...
	 (given & RESERVED_FORMAT_MASK))
	{
      ...
      break;
    default: 
      fprintf(stdout, "What the hell is this? The insn doesn't have a type. insn->i = %08x insn->name = %s  insn->type = %08x\n",
