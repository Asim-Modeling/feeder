     ...
     IFEEDER_BASE_CLASS("Trace Feeder")
{
    ...
    if (MT_EXIT_INSTS == 0)
    {
	   mtInstLimit = UINT64_MAX;
    }
    else
    {
	   mtInstLimit = UINT64(MT_EXIT_INSTS);
    }

    mtThreadsRunningVector = 0;
    mtExitCondition = 0;
    ...
    char **argv)
{
	...
	if (!strcmp(argv[i], "-i")) {
	    mtInstLimit = strtoul(argv[i+1],NULL,0);
	    if (mtInstLimit) {
    ...
    ASIM_INST inst)
{
    ...
    {
        T1("Context switch requested.  Creating a NOP to represent a context switch.");
    ...
    if(trace[streamId].Eof() || mtExitCondition)
    {
        pc = predicted_pc;
        T1("\tEnding thread, uid = " << streamId.Handle()); 
        T1("\tSkipped " << totalSkippedWrongpath
    ...
    {
        T1("\tAttempt to fetch IP 0 due to thread startup.  Creating BR starting trace IP.");
    ...
    ASIM_INST inst)
{
    ...
    UINT32 skipped = trace[sid].CommitInstr(inst->GetTraceID());
...
//    }
    ...
    //
    //  Check to see if all threads have completed mtInstLimit instructions
    //
    if (mtInstLimit) {
	UINT32 tpu = inst->GetSWC()->GetHWCNum();
	committedInsts[tpu] += skipped + 1;

	if ((mtThreadsRunningVector & (1 << tpu)) && (committedInsts[tpu] >= mtInstLimit)) {

	    // clear the bit corresponding to this thread
	    mtThreadsRunningVector &= ~(1 << tpu);
	}

	if (mtThreadsRunningVector == 0) {
	    mtExitCondition = 1;
	}
    }
}
    ...
    const bool killMe)
{
    ...
    T1("\tFEED_Kill id=" << inst->GetTraceID()
          << " fetchNext=" << fetchNext
          << " killMe=" << killMe);
    ...
    char **envp)
{
	...
	committedInsts[t] = 0;
	mtThreadsRunningVector |= (1 << t);  // Mark this thread as not yet ready to quit
        ...
        ARCH_REGISTER reg)
{
    ...
    T1("\tcheck in feeder valid: "<<reg->IsValid()<<" type: "<<reg->GetType()<<" number"<<reg->GetNum());
