...
#ifndef __DISASM_TEST_H__
#define __DISASM_TEST_H__ 1
...
extern char *disassemble(unsigned int encoded, u64 pc);
extern char *decode_memop(unsigned int encoded, unsigned long pc, int *src1, int *src2, int *offset);

static int IsLoadStore(uint inst) {
  ...
  uint opcode = OPCODE(inst);
  return((int) ( ((uint64) UINT64_CONST(0x0000ffff0000fc00)  >> (uint64) opcode) &
                 (uint64)1 ));
}


static int IsLoad(uint inst) {
  /* True for LDQ_U       LDF/G/S/T            LDL/Q/L_L/Q_L                */
  /*          0b          20..27               28..2f  */
  /* False for all others, including LDA LDAH MISC JSR  */
  /* False for PAL LD/ST physical, etc. */
  uint opcode = OPCODE(inst);
  return((int)( ((uint64) UINT64_CONST(0x00000f0f00001c00) >> (uint64) opcode) &
                (uint64) 1 ));
}


static int IsLlSc(uint inst) {
  /* True for LDQ_U STQ_U LDF/G/S/T ST F/G/S/T LDL/Q/L_L/Q_L STL/Q/L_C/Q_C  */
  /*          0b    0f    20..27               28..2f  */
  /* False for all others, including LDA LDAH MISC JSR  */
  /* False for PAL LD/ST physical, etc. */
  uint opcode = OPCODE(inst);
  return((int)( ((uint64) UINT64_CONST(0x0000cc0000000000) >> (uint64)opcode) & (uint64)1 ));
}


static int IsStore(uint inst) {
  /* True for LDQ_U STQ_U LDF/G/S/T ST F/G/S/T LDL/Q/L_L/Q_L STL/Q/L_C/Q_C  */
  /*          0b    0f    20..27               28..2f  */
  /* False for all others, including LDA LDAH MISC JSR  */
  /* False for PAL LD/ST physical, etc. */
  uint opcode = OPCODE(inst);
  return((int)( ((uint64) UINT64_CONST(0x0000f0f00000e000) >> (uint64)opcode) & (uint64)1 ));
}

static char rwchar(uint inst) {
  /* True for LDQ_U STQ_U LDF/G/S/T ST F/G/S/T LDL/Q/L_L/Q_L STL/Q/L_C/Q_C  */
  /*          0b    0f    20..27               28..2f  */
  /* False for all others, including LDA LDAH MISC JSR  */
  /* False for PAL LD/ST physical, etc. */
