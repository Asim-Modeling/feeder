...
IsLoad(UINT64 inst) 
{
  ...
  // False for PAL LD/ST physical, etc.
  UINT64 opcode = OPCODE(inst);
  return((INT64) ( ((UINT64) UINT64_CONST(0x00000f0f00001c00) >> (UINT64) opcode) &
		   (UINT64)1 ));
}
...
IsLlSc(UINT64 inst) 
{
  ...
  // False for PAL LD/ST physical, etc.
  UINT64 opcode = OPCODE(inst);
  return((INT64) ( ((UINT64) UINT64_CONST(0x0000cc0000000000) >> (UINT64) opcode) &
		   (UINT64)1 ));
}
...
IsStore(UINT64 inst) 
{
  ...
  // False for PAL LD/ST physical, etc.
  UINT64 opcode = OPCODE(inst);
  return((INT64) ( ((UINT64) UINT64_CONST(0x0000f0f00000e000) >> (UINT64) opcode) &
		   (UINT64)1 ));
}
...
rwchar(UINT64 inst) 
{
  ...
  // False for PAL LD/ST physical, etc.
  UINT64 opcode = OPCODE(inst);
  return("...........R...W................rRrRwWwWrRlLwWuU................"[opcode]);
}
...
void ATF_tlb_dump_dbg (Address va)
{
	...
	tlbe_ptr = &atf_stream[0].cpu_ptr->tlb[ent];
        if (tlbe_ptr->vpage == PAGE_ADDR_PAGE(va))
            /*CPUWarning("Found in ATF TLB ent=%3d vp="FMT64X" pte="FMT64X" asn="FMT64X" *************\n",*/
            fprintf(stderr, "Found in ATF TLB ent=%3d vp="FMT64X" pte="FMT64X" asn="FMT64X" *************\n",
...
{
    Atf_Stream *ast_ptr = &atf_stream[stream_id];
    if (! (varnum>avl_MAX && varnum<avg_MAX)) {
       msg("Bad global variable number %d... Maybe local?", varnum);
       exit (1);
    }
...
{
    Atf_Stream *ast_ptr = &atf_stream[stream_id];
    if (! (varnum>=0 && varnum<avl_MAX)) {
       msg("Bad local variable number... Maybe global?", varnum);
       exit (1);
    }
    ...
    /*		trailing :# to indicate filtering to a specific CPU. */
{
    ...
    if (! (stream_id>=0 && stream_id<ATF_STREAMS_MAX)) {
       printf("Illegal stream id.");
       exit(1);
    }
...
AtfAction_enum ATF_read_action (int stream_id,
				AtfGenParam *value1_ptr, AtfGenParam *value2_ptr, AtfGenParam *value3_ptr, AtfGenParam *value4_ptr)
    /* Read the next token(s) from the file, and return what it did */
{
	...
	return_instr_old:
	    value1_ptr->uint64 = cpu_ptr->vpc;
	    value2_ptr->ptr    = &(aie_ptr->opcode_etc);
	    value3_ptr->uint64 = last_vea;
	    atf_set_pc (ast_ptr, 0x4, TRUE/*relative*/);
	...
	if (ast_ptr->is_dumping) {
		...
		value1_ptr->uint64 = pa & ~0x3f;
		value2_ptr->uint64 = va;
		value3_ptr->uint64 = pa & 0x1;
		value4_ptr->uint64 = (pa >> 3) & 0x3;
		return (act_DUMP);
	    } else {
		/* End dump */
		ast_ptr->is_dumping = FALSE;
	...
	if ((token & 0xe0) == aft_EXEC_SEQ) {
	    ...
	    value1_ptr->uint64 = avg_CPU;
	    value2_ptr->uint64 = token & 0x0f;
	    atf_set_variable (ast_ptr, (AtfVariable_enum)value1_ptr->uint64, value2_ptr->uint64);
	    ...
	    case aft_SET_REL:
		value1_ptr->uint64 = get_byte(ast_ptr);
		byte = get_byte(ast_ptr);
		value2_ptr->uint64 = VALUE_DELTA1 (byte) + atf_get_variable (ast_ptr, (AtfVariable_enum)value1_ptr->uint64);
		atf_set_variable (ast_ptr, (AtfVariable_enum)value1_ptr->uint64, value2_ptr->uint64);
		DBG (ATF_DEBUG, 0, msg ("\taft_SET_REL %lx %lx\n", value1_ptr->uint64, value2_ptr->uint64));
		return (act_SET);
		
	    case aft_SET_EXACT:
		value1_ptr->uint64 = get_byte(ast_ptr);
		value2_ptr->uint64 = get_qw(ast_ptr);
		atf_set_variable (ast_ptr, (AtfVariable_enum)value1_ptr->uint64, value2_ptr->uint64);
		DBG (ATF_DEBUG, 0, msg ("\taft_SET_EXACT %lx %lx\n", value1_ptr->uint64, value2_ptr->uint64));
		return (act_SET);
		
	    case aft_INC_EPOCH:
		value1_ptr->uint64 = avg_EPOCH;
		value2_ptr->uint64 = ast_ptr->global_var[ avg_EPOCH ] + 1;
		atf_set_variable (ast_ptr, (AtfVariable_enum)value1_ptr->uint64, value2_ptr->uint64);
	    ...
	    case aft_PTE:
		delta = get_qw(ast_ptr);
		value1_ptr->uint64 = PAGE_ADDR_PAGE(delta);
		value2_ptr->uint64 = get_qw(ast_ptr);
		atf_tlb_fill_entry (ast_ptr, value1_ptr->uint64, value2_ptr->uint64, PAGE_ADDR_OFFSET(delta));
		DBG (ATF_DEBUG, 0, msg ("\taft_PTE %lx %lx\n", value1_ptr->uint64, value2_ptr->uint64));
		RETURN_OR_CONTINUE (act_PTE);
		
	    case aft_DMA:
		value1_ptr->uint64 = get_qw(ast_ptr);
		value2_ptr->uint64 = get_lw(ast_ptr);
		value3_ptr->uint64 = get_byte(ast_ptr);
		DBG (ATF_DEBUG, 0, msg ("\taft_DMA %lx %lx %lx\n", value1_ptr->uint64, value2_ptr->uint64, value3_ptr->uint64));
		...
		case acm_PROC_TEXT_INFO:
		    ...
		    value1_ptr->uint64 = get_qw (ast_ptr); /* pid */
		    value2_ptr->uint64 = get_qw (ast_ptr); /* start */
		    value3_ptr->uint64 = get_qw (ast_ptr); /* end */
		    string = atf_get_string (ast_ptr);
		    value4_ptr->ptr = string;    /* uu_comm */
		...
		case acm_THREAD_USTACK_ADDR_INFO:
		    ...
		    value1_ptr->uint64 = get_qw (ast_ptr); /* pid */
		    value2_ptr->uint64 = get_qw (ast_ptr); /* tid */
		    value3_ptr->uint64 = get_qw (ast_ptr); /* start */
		    value4_ptr->uint64 = get_qw (ast_ptr); /* end */
		...
		case acm_THREAD_USTACK_GROW_INFO:
		    ...
		    value1_ptr->uint64 = get_qw (ast_ptr); /* pid */
		    value2_ptr->uint64 = get_qw (ast_ptr); /* tid */
		    value3_ptr->uint64 = get_qw (ast_ptr); /* size */
		    value4_ptr->uint64 = get_byte (ast_ptr); /* uu_grow_up */
		...
		case acm_LIBLOADER_INFO:
		    ...
		    value1_ptr->uint64 = get_qw (ast_ptr); /* text start */
		    value2_ptr->uint64 = get_qw (ast_ptr); /* text end */
		    value3_ptr->uint64 = get_qw (ast_ptr); /* bss start */
		    value4_ptr->uint64 = get_qw (ast_ptr); /* bss end */
		...
		case acm_KERNEL_SWITCH:
		    ...
		    value1_ptr->uint64 = get_byte (ast_ptr); /* in */
		    value2_ptr->uint64 = get_qw (ast_ptr);   /* addr */
		    string = atf_get_string (ast_ptr);
		    value3_ptr->ptr = string;      /* comment */
		    return(act_KERNEL_SWITCH);

		case acm_IDLE_THREAD:
		    DBG (ATF_DEBUG, 0, msg ("\tacm_IDLE_THREAD\n"));
		    value1_ptr->uint64 = get_byte (ast_ptr); /* start */
